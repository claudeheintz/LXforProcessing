...
// ***** MIDI
//       select mode with radio group rgMIDI
static final int MIDI_OFF = 0;
static final int MIDI_ENABLED = 1;
...
void setup() {
  ...
  */
  ...
  rgMIDI = new LXPRadioGroup(2);
  nrb = rgMIDI.addButton(100, 365, 20);
  nrb.title = "OFF";
  nrb = rgMIDI.addButton(100, 325, 20);
  nrb.title = "MIDI";
  nrb.state = true;
  rgMIDI.setSelectedIndex(MIDI_OFF);      //MIDI starts off
...
void mousePressed() {
  if ( setup_mode ) {
    ...
    } else if ( rgMIDI.mousePressed() ) {
      if ( rgMIDI.selected == MIDI_ENABLED ) {
        initMIDI();
      } else {
        myBus = null;
      }
...
void initMIDI() {
  ...
  if ( myBus == null ) {
    rgMIDI.setSelectedIndex(MIDI_OFF);
  }
}
